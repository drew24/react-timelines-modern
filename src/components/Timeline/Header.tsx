import React, { ComponentPropsWithoutRef, MouseEvent, PureComponent, RefObject } from 'react'
...
class Header extends PureComponent<Props> {
  ...
  componentDidMount() {
    ...
    if (sticky) {
      const currentOffsetHeight = this.timebar.current ? this.timebar.current.offsetHeight : 0;
  ...
  handleScroll = () => {
    const { sticky } = this.props
    const targetScrollY = this.scroll.current ? this.scroll.current.scrollLeft : 0;
